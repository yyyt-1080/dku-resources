...
The live URL above is encoded as a **static QR code** (not a dynamic/redirect code), so it has no expiration and doesn't depend on any third-party account. If the URL ever changes (e.g. repo renamed, GitHub username changed), a new QR code must be generated and reprinted.
